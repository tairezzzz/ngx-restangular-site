import {
    Directive, OnInit, Inject, PLATFORM_ID, Input, HostListener, ElementRef, Renderer2
} from '@angular/core';
            ...
            for (let i = 0; i < this.parallaxData.length; i++) {
                if (this.el.nativeElement.id === this.parallaxData[i].id ) {
                    ...
                    this.parallaxData[i].count = 
                        (this._svgBgImage.getBoundingClientRect().top - parseInt((this.parallaxData[i].adjustmentPos), 10))
                        / parseInt((this.parallaxData[i].state), 10) + '%';
                    this.renderer.setStyle( this.el.nativeElement, 'transform', 'translateY(' + this.parallaxData[i].count + ' )');
                }
            }
        }
    }
